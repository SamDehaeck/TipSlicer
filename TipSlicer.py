...
def loadStl(stlName,mulDimension=1.,centred=True,zToZero=True):
...
def singleSliceStl(mesh,Zmins,Zmaxs,Zslice,X,Y,epsilon=0.001,onlyShell=False):
...
def getHatching_old_old(sli,X,Y,direction='X',crossed=False,debug=False):
	if (direction=='X'):
		tesBig=np.zeros([sli.shape[0],sli.shape[1]+2],dtype='bool')
		tesBig[:,1:-1]=sli
		...
		tesBig=np.zeros([sli.shape[0]+2,sli.shape[1]],dtype='bool')
		tesBig[1:-1,:]=sli
		edgeXStart=(tesBig[1:]>tesBig[:-1])
		edgeXEnd=(tesBig[1:]<tesBig[:-1])
		SS=np.argwhere(edgeXStart.T)
		SS=SS[:,::-1]
		EE=np.argwhere(edgeXEnd.T)
		EE[:,1]=EE[:,1]-1
		EE=EE[:,::-1]
	stCoX=X[SS[:,0],SS[:,1]]
	stCoY=Y[SS[:,0],SS[:,1]]
	endCoX=X[EE[:,0],EE[:,1]]
	endCoY=Y[EE[:,0],EE[:,1]]
	ordArray=np.zeros((len(stCoX),4))
	if (debug):
		return SS,EE,stCoX,stCoY,endCoX,endCoY
	if (not crossed):
		for i in np.arange(len(stCoX)):
			ordArray[i]=np.array([stCoX[i],stCoY[i],endCoX[i],endCoY[i]])
	else:
		if (direction=='X'):
			unSS=unique(SS[:,0])
			lineBreaks=np.asarray([argwhere(SS[:,0]==unVal)[0][0] for unVal in unSS])
			ordInd=0
			for lb in np.arange(len(lineBreaks)-1):
				indRange=[lineBreaks[lb],lineBreaks[lb+1]]
				if (lb%2==0):
					for ii in arange(indRange[0],indRange[1]):
						ordArray[ordInd]=np.array([stCoX[ii],stCoY[ii],endCoX[ii],endCoY[ii]])
						ordInd=ordInd+1
				else:
					for ii in arange(indRange[1]-1,indRange[0]-1,-1):
						ordArray[ordInd]=np.array([endCoX[ii],endCoY[ii],stCoX[ii],stCoY[ii]])
						ordInd=ordInd+1
			# now do last line
			for ii in arange(lineBreaks[-1],len(stCoX)):
						ordArray[ordInd]=np.array([stCoX[ii],stCoY[ii],endCoX[ii],endCoY[ii]])
						ordInd=ordInd+1
...
def getHatching_old(sli,X,Y,z,direction='X',crossed=False,debug=False):
	if (direction=='X'):
		tesBig=np.zeros([sli.shape[0],sli.shape[1]+2],dtype='bool')
		tesBig[:,1:-1]=sli
		edgeXStart=(tesBig[:,1:]>tesBig[:,:-1])
...
# This will write all x,y points with the intensity given in sli to give an intensity modulation
# random: not just scanning 'left to right' but randomise line order
def getHatchingGrayScale(sli,X,Y,z,angle=0,subdiv=1,crossed=True,random=False,debug=False):
	Ir=im.rotate(sli,angle,order=0)[::subdiv,::subdiv] # subdivide in rows and cols
	Xr=im.rotate(X,angle)[::subdiv,::subdiv]
	Yr=im.rotate(Y,angle)[::subdiv,::subdiv]
	if (Ir.sum()==0): # After the subdivide there can be nothing left..
		return [],[]

	ordList=[]
	distanceList=[]                  # keep track of the written length for time estimation
	inds=np.arange(Xr.shape[0])
	if (random):
	...
	if (not crossed):
		for i in inds:
			...
			if (len(gI[0])>1):
				...
				yi=Yr[i][gI]
				Ii=Ir[i][gI]
				ordList.append(np.column_stack((xi,yi,z*np.ones(len(xi)),Ii)))
				distanceList.append(np.sqrt((xi[-1]-xi[0])**2+(yi[-1]-yi[0])**2))
	else:
		for i in inds:
			gI=np.where(Ir[i]>0)
			if (len(gI[0])>1):
				...
				yi=Yr[i][gI]
				Ii=Ir[i][gI]
				writeA=np.column_stack((xi,yi,z*np.ones(len(xi)),Ii))
				distanceList.append(np.sqrt((xi[-1]-xi[0])**2+(yi[-1]-yi[0])**2))
				if (i%2==0):
					ordList.append(writeA)
				else:
					ordList.append(writeA[::-1])
	return ordList,np.sum(distanceList)
...
def stackToGwl(stackName,gwlName,amShells,scaffoldStep,hatchStep,atAngles=[0,90,180,270],codeSpeeds=[10000,10000,10000],codeIntens=[80,80,80],sliceIndexList=[],contApprox=['L1'],divider=1,randomShell=True,doTopBottom=True,writeHeader=True,writeColourH5=False,interfacePos=-1):
	...
	writeGwlFile(gwlName,allSliceList,allSliceCodes,allSliceZList,codeSpeeds,codeIntens,stagePosition=GC,writeHeader=writeHeader,interfacePos=interfacePos,debug=True)
	if (writeColourH5):
		Z2=Z[sliceIndexList]
...
def writeGwlForRegex(outGwlName,filePattern,codeSpeeds=[10000,10000,10000],codeIntensities=[80,80,80],interfacePos=-1,debug=True):
	...
	with open(outGwlName,mode="w") as F:
		...
		for ff in fnames:
			...
			F.write('include {}\r'.format(os.path.relpath(ff).replace('/','\\')))
...
def getVoteCentre_freestanding(T,L,Vote,WriteRad,interior=True):
    ...
    return np.array([xmax,ymax]),T2,goodLabs
...
def getOptimalFreestandingStitchCentres(T,VoteRadius,WriteRadius,X,Y,closing=0):
    ...
    while(In.sum()!=0):
     ...
     #   VoteList.append(Vote)
        cp,Out,goodLabs=getVoteCentre_freestanding(In,L,Vote,WriteRad_pix,True)
        diff=(In-Out).sum()
        if (diff==0): # something went wrong with differential voting => go back to absolute voting
            print('Something wrong with voting, will not close image')
            Vote=getVoteFullCircle(In,VoteRad_pix)
            cp,Out,goodLabs=getVoteCentre_freestanding(In,L,Vote,WriteRad_pix,True)
            diff=(In-Out).sum()
            if (diff==0):
